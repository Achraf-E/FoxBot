    ...
    async run(bot, interaction, options) {
        interaction.guild.members.fetch(options.get("membre")).then(member => {            
            ...
            if(member.roles.highest.comparePositionTo(interaction.member.roles.highest) > -1){
                interaction.guild.channels.cache.get("1130483351199961219").send(` \`${interaction.user.username}\` a essayé de unmute \`${member.user.username}\` mais n'en a pas les droits.`);
            ...
            member.timeout(1, (options.get("raison")? options.get("raison").value:"Pas de raisons")).then(() => {
                interaction.guild.channels.cache.get("1130483351199961219").send(` \`${interaction.user.username}\` a unmute \`${member.user.username}\` pour la raison : \`${options.get("raison")? options.get("raison").value:"Pas de raisons"}\``);
