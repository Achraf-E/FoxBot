    ...
    async run(bot, interaction, options) {
        interaction.guild.members.fetch(options.get("membre")).then(member => {
            ...
            if(member.roles.highest.comparePositionTo(interaction.member.roles.highest) > -1){
                ...
                return interaction.reply(`Vous ne pouvez mute \`${member.user.username}\`, les logs on été envoyé dans le channel des moderateurs`);
            }
            //We can mute
            member.timeout(options.get("temps").value * 1000, options.get("raison").value).then(() => {
                ...
                return interaction.reply(`Vous avez bien mute \`${member.user.username}\`, les logs ont été envoyé dans le channel des moderateurs`)
