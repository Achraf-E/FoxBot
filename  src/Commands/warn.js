    ...
    async run(bot, interaction, options , db){
        var member = interaction.guild.members.fetch(options.get("membre")).then(async member => {
            ...
            if(member.roles.highest.comparePositionTo(interaction.member.roles.highest) > -1){
                interaction.guild.channels.cache.get("1130483351199961219").send(` \`${interaction.user.username}\` a essayé de warn \`${member.user.username}\` mais n'en a pas les droits.`);
            ...
            interaction.guild.channels.cache.get("1130483351199961219").send(` \`${interaction.user.username}\` a warn \`${member.user.username}\` pour la raison : \`${options.get("raison").value}\``);
            ...
            console.log(err);
            interaction.guild.channels.cache.get("1130483351199961219").send(` \`${interaction.user.username}\` a essayé de warn \`${options.getUser("membre")}\` mais une erreur est survenue (est-il dans le serveur?).`);
...
function checkWarns(bot,interaction,member,db){
    db.query(`SELECT count(*) AS count FROM warns WHERE warned = '${member.user.username}'`, function(err, result, fields){
        ...
        if(count > 2){
            ...
            interaction.guild.channels.cache.get("1130483351199961219").send({embeds: [embed]});
