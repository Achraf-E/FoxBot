...
module.exports = {
    ...
    async run(bot, interaction, options){ 
        ...
        interaction.guild.members.fetch(options.get("membre")).then(async member => {
            ...
            interaction.guild.channels.cache.get("1130483351199961219").send(` \`${interaction.user.username}\` a essayé de ban \`${options.getUser("membre")}\` mais une erreur est survenue (est-il dans le serveur?).`);
