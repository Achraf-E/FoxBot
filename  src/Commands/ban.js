    ...
    async run(bot, interaction, options){ 
        ...
        interaction.guild.members.fetch(options.get("membre")).then(async member => {
                        ...
                        if(member.roles.highest.comparePositionTo(interaction.member.roles.highest) > -1){
                            ...
                            return interaction.reply({ephemeral: true, content :  `Vous ne pouvez ban \`${member.user.username}\`, les logs on été envoyé dans le channel des moderateurs`}`Vous ne pouvez ban \`${member.user.username}\`, les logs on été envoyé dans le channel des moderateurs`);
                        }
