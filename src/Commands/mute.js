...
module.exports = {
    ...
    async run(bot, interaction, options) {
        interaction.guild.members.fetch(options.get("membre")).then(member => {
            //official if the person can mute
            ...
            member.timeout(options.get("temps").value * 1000, options.get("raison").value).then(() => {
                interaction.guild.channels.cache.get(config.moderate_channel).send(` \`${interaction.user.username}\` a mute \`${member.user.username}\`  \`${options.get("temps").value}seondes\` pour la raison : \`${options.get("raison").value}\``);
